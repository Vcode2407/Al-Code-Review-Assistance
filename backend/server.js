...
app.post('/review', verifyToken, authorizeRoles('user', 'admin'), async (req, res) => {
  try {
    ...
    console.error("FULL ERROR:", error); res.status(500).json({ error: "Audit failed", reason: error.message });
  }
});
...
// Force Update 
// Deployment Sync v2 
app.get("/env-test", (req, res) => { res.json({ hasJwtSecret: !!process.env.JWT_SECRET, hasGeminiKey: !!process.env.GEMINI_API_KEY }); });
